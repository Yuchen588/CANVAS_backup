...
│   └── feature_selection_modeling.R   # Performs Bootstrap LASSO, random forest, and Cox regression
│
└── Downstream_Analysis/          # Module 4: Downstream biological, spatial, and clinical analyses
    │
    ├── Figure1/                  # Cell composition, annotation, and compartment-level analyses
    │   ├── Cell_Type_Annotation_in_the_WSI_Cohort.R   # Cell type annotation in the WSI cohort
    ...
    │   ├── Cell_Type_Fraction_Comparison.R            # Compares cell-type fractions across cohorts/groups
    │   ├── Cellular_Compartment_Comparison.R          # Compares major cellular compartments
    │   └── markers.csv                                # Marker reference table for annotation
    │
    ├── Figure2/                  # Cell interaction, co-occurrence, and network motif analyses
    │   ├── Cell_Cell_Co_occurrence.R                  # Quantifies spatial co-occurrence between cell types
    │   ├── Cell_Cell_Interaction.txt                  # Generates or summarizes cell-cell interaction analysis
    │   ├── Cell_Type_Clinical_Parameters.R            # Associates cell features with clinical parameters
    │   └── GNN_Based_Cell_Network_Triplet_Motif_Analysis.txt
    │                                                  # GNN-based triplet motif and cell network analysis
    │
    ├── Figure3/                  # Neighborhood, distance, ecological unit, and proximity analyses
    │   ├── Cell_Cell_Distance_Analysis.R              # Analyzes pairwise spatial distances between cell types
    ...
    │   ├── Distance_and_Proximity_Density_Generation.txt
    │                                                  # Generates distance and proximity-density features
    │   └── Proximity_Density_Analysis.R               # Analyzes proximity-density relationships
    │
    ├── Figure4/                  # CANVAS model performance and molecular/clinical associations
    │   ├── CANVAS_model.txt                          # CANVAS model architecture or implementation notes
    │   ├── CANVAS_training.txt                       # CANVAS training procedure and settings
    │   ├── CN_Assignment.txt                         # CN assignment workflow and documentation
    │   ├── CNV_and_SNV_Associations.R                # Associates habitats/features with CNV and SNV
    ...
    └── Figure5/                  # Habitat-derived prognostic feature construction and modeling
        ...
        ├── Feature_Distance.txt                      # Habitat distance feature generation
        ├── Feature_Diversity.R                       # Diversity feature analysis
        ├── Feature_Entropy.R                         # Entropy / STE feature analysis
